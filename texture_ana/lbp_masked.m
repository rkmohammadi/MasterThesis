...
function result = lbp_masked(varargin) % image,mask,radius,neighbors,mapping,mode)
% Version 0.3.2
...
mask = logical(varargin{2}); % EDIT: binary mask as input
...
if (nargin > 1) && (length(varargin{3}) > 1)
    spoints=varargin{3};
...
if (strcmp(mode,'h') || strcmp(mode,'hist') || strcmp(mode,'nh'))
    ...
    % EDIT get possible points for local pattern computations based on the 
    % binary mask
    mask = imerode(mask,strel('disk',round(radius)));
    
    maskSmall = mask(origy:origy+dy,origx:origx+dx);
